...
<div class="bg-white p-6 rounded-lg shadow-md">
    ...
    <form method="GET" class="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
        ...
        <div class="flex space-x-2">
            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerlad-700 text-white font-bold py-2 px-4 rounded-md">Filter</button>
            ...
            <?php if (mysqli_num_rows($result) > 0): ?>
                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4">
                            <div class="font-semibold text-gray-800"><?php echo htmlspecialchars($row['nama_praktikum']); ?></div>
                        ...
                        <td class="py-3 px-4 text-center">
                            <?php if ($row['id_penilaian']): ?>
                                <span class="bg-green-100 text-green-800 text-xs font-medium px-3 py-1 rounded-full shadow-sm">Sudah Dinilai</span>
                            <?php else: ?>
                                <span class="bg-yellow-200 text-yellow-800 text-xs font-medium px-3 py-1 rounded-full shadow-sm">Belum Dinilai</span>
                            <?php endif; ?>
                        </td>
                        <td class="py-3 px-4 text-center">
                            <a href="detail_laporan.php?id=<?php echo $row['id']; ?>"
                                class="bg-blue-600 hover:bg-blue-700 text-white text-xs px-4 py-2 rounded-md font-semibold shadow-sm">
                                Detail & Nilai
                                </a>
                            </td>
                        </tr>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="py-6 px-4 text-center text-gray-500 italic">Tidak ada laporan yang sesuai dengan filter.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
